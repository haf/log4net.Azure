...
namespace :env do
  ...
  task :set_dirs do
    ...
    FOLDERS[:az][:out] = File.join(FOLDERS[:src], PROJECTS[:az][:dir], 'bin', CONFIGURATION)
    CLEAN.include(FOLDERS[:az][:out])

    # for tests
    FOLDERS[:az][:test_out] = File.join(FOLDERS[:src], PROJECTS[:az][:test_dir], 'bin', CONFIGURATION)
    FILES[:az][:test] = File.join(FOLDERS[:az][:test_out], "#{PROJECTS[:az][:test_dir]}.dll")
